...
class SpectrumSnapshotWriter(object):
    ...
    def writeToFile(self, filePath, snapshot, header, spectrum):
        with open(filePath, "w") as f:
            ...
            f.write("%d " % snapshot.totalPoints) #总点数
...
class ApplicationWindow(QMainWindow):
    ...
    def bind(self, snapshot):
        def Gene():
            tick = time.time()
            userArgs = {"ant":1}
            header, spectrum = snapshot.gene(self.editStartFreq.frequency, self.editStopFreq.frequency, self.editRBW.frequency, **userArgs)
