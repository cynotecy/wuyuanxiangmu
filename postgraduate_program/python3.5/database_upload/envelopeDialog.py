# -*- coding: utf-8 -*-
from Ui.UitoPy.Ui_envelope_data import Ui_Dialog
...
class MainWindow(QDialog, Ui_Dialog):
    ...
    def on_pushButton_clicked_1(self):
        filename, _ = QFileDialog.getOpenFileName(self, "选择文件", r"..\specEnvelope_recvfiles", )
        print(filename)
    ...
    def on_pushButton_clicked_3(self):
        ...
        if (
            ...
            print(type(name))
            try:
                start_freq = float(self.lineEdit_10.text())
            except:
                QMessageBox.information(self, "提示", "起始频率必须为数字")
            # print(type(bdwth))
            else:
                try:
                    end_freq = float(self.lineEdit_9.text())
                except:
                    QMessageBox.information(self, "提示", "截止频率必须为数字")
                else:
                    # print(type(ctrfrq))
                    ...
                    insert = 'INSERT INTO `freq_data`(`name`, `item`, `create_time`, `modulation`' \
                             ', `place`, `data`, `remarks`, `device`, `frequencyl`, `frequencyr`' \
                             ') VALUES ("%s", "%s", %s, "%s", "%s", "%s",' \
                             ' "%s", "%s", "%s", "%s")'%(name, items, createTime, modulation, place,
                                                                 datafile, remarks, device, start_freq, end_freq) # 新增SQL语句
                    print(insert)
                    # try:
