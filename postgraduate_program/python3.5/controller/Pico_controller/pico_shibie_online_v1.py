...
def compute_glcm_metrics(p, p_x,  p_y, p_xpy, p_xmy, N_g):
    ...
    '''
    因分母为0导致结果出现inf，所以分母加1的偏置量
    '''
    metrics_vect['Inverse Difference moment'] = np.sum((1. / (2+((ndr.reshape([-1, ]) - ndc.reshape([-1, ])) * p.reshape([-1, ])))))
...
def extract_feature(path, save_path):
    all_features = []
    all_labels = []
    ...
    for each_path, label in inputs[:20]:
        one_signal_feature = []
    ...
    features_array = normalization(features_array, axis=0)
    feature_and_label = np.concatenate((features_array, labels_array), axis=1)

    # print(feature_and_label.shape)

    df = pd.DataFrame(feature_and_label)
    df.to_csv(save_path, header=False, index=False, sep=',')

    def text_feature(data, label):
        one_signal_feature = []
        data = np.loadtxt(each_path).astype(np.float32)
        ...
        # print(' one_signal_feature:',  one_signal_feature)
        ...
        features_array = np.array(all_features, dtype=np.float32)
        features_array = normalization(features_array, axis=0)
        # features_array = max_normalization(features_array, axis=0)

        feature_and_label = np.concatenate((features_array, labels_array), axis=1)
        df = pd.DataFrame(feature_and_label)
        df.to_csv(save_path, header=False, index=False, sep=',')

def extract_feature2(path,save_path):
    all_features = []
    all_labels = []
    ...
    for each_path, label in inputs:
        ...
        data = data1.T
        # print(data.shape)
        # print('each_signal_size:',data1.shape)
        # 归一化，之前进行了功率归一化
...
def sum_feature_extraction(path,save_path): #提取很多特征，并且打标签
    all_features = []
    all_labels = []
    inputs = np.loadtxt(path, str)
    # indices = np.arange(len(inputs))
    ...
    for each_path, label in inputs:
        # print(each_path)
        texture_features, texture_labels = texture_feature_extraction(each_path, label)
        general_feature, general_labels = general_feature_extraction(each_path, label)

        sum_feature =  general_feature + texture_features

        all_labels.append([label])
        all_features.append(sum_feature)
        ...
        df = pd.DataFrame(feature_and_label,columns=get_index())
        # print(df)
    return df

def sum_feature_extraction_only(path,save_path):  #只是输入一个信号提取特征，并且不打标签的状况
    all_features = []
    all_labels = []
    inputs = np.loadtxt(path, str)
    for each_path, label in inputs:
...
def get_normalized_data_oneSignal(input_data):
    ...
    data_path = r'controller\Pico_controller\history_feature_data\new_pico_feature.csv'
...
def get_testdata(root_path, length, test_path):
    '''
...
def recongnize(test_path):
    ...
    for each_path in input:
        ...
        model_path = father_path + r'\controller\Pico_controller\model_saved\new_AdaBoost_GBDT.py'  # 获取模型所在目录
    ...
    return pre_result
def remove_files(txt_path,test_path):
    '''
    函数功能：删除txt_path文件夹里的所有文件及test_path文件
...
def configuration(mat_path,length,class_name = 'test_device'):
    ...
    get_testdata(txt_path, length, test_path)
    pre_result = recongnize(test_path)  # 函数内打印了当前处理文件的路径
    pre_result = max_voter(pre_result)
    remove_files(txt_path, test_path)
    return pre_result
# mat_path = r"..\pico_jicheng_online\test_data_mat"
# length = 10
