...
def test_model(model, inputs):
    ...
    with torch.no_grad():
        ...
        elif signal_freq in Interval(825, 835):  # CDMA800上行
        ...
        elif signal_freq in Interval(870, 880):  # CDMA800下行  s=25M
        ...
        elif signal_freq in Interval(885, 890):  # EGSM900上行  s=2M
            radio_name = 'EGSM900上行'
            u = 1  # !!!待修改
            prior_p[:, 13] = 5

        elif signal_freq in Interval(890, 909):  # GSM900上行   s=25M
            radio_name = 'GSM900上行'
            u = 1  # !!!待修改
            prior_p[:, 13] = 5
        ...
        elif signal_freq in Interval(930, 935):  # EGSM900下行   s=25M
        ...
        elif signal_freq in Interval(1710, 1745):  # 移动GSM1800    s=25M
            radio_name = 'GSM1800上行'
            u = P / 1.92e-6  # !!!待修改
            prior_p[:, 13] = 5

        elif signal_freq in Interval(1755, 1780):  # FDD-LTE上行
        ...
        elif signal_freq in Interval(1920, 1935):  # 电信CDMA2000      f=1829.4,s=2M,b=0.2e6
        ...
        if len(top_3) == 2:
            ...
            reslt_list.append(str(signal_freq))
            reslt_list.append(str(bandwidth))
            string = "%s:%s%%,%s:%s%%" % (str(classes[top_3[0][0]]), str(round(int(top_3[0][1]) / batch_size * 100, 2)),
        ...
        if len(top_3) == 3:
            ...
            reslt_list.append(str(signal_freq))
            reslt_list.append(str(bandwidth))
            string = "%s:%s%%,%s:%s%%,%s:%s%%" %(str(classes[top_3[0][0]]), str(round(int(top_3[0][1]) / batch_size * 100, 2)),
                                              str(classes[top_3[1][0]]), str(round(int(top_3[1][1]) / batch_size * 100, 2)),
...
def test_(file_path=r'..\usrp_recvfiles\auto_recognize'):
    ...
    import time
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), ': load model!')
    net.load_state_dict(torch.load(logdir))
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), ': load finish!')
    model = net.eval()
    result = []
