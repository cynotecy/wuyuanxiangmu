'''
程序说明：
(1)该程序是用于识别未知设备的开关脉冲信号以定位干扰源，训练样本为4种设备
(2)界面只需调用 configuration(class_name,mat_path,txt_path,length,test_path)函数进行配置
'''
...
def data_deal():
    ...
    data_path = r'..\python3.5\controller\Pico_controller\history_feature_data\raw_data_three_demo.csv'
    data = pd.read_csv(data_path, usecols=index)
    data[np.isnan(data)] = 0
    data_numpy = data.values
    data_15_column_sum = 0
    data_16_column_sum = 0
    data_32_column_sum = 0
    num = 0
    for i in data_numpy[:, 15]:
        if math.isnan(i):
            pass
        else:
            data_15_column_sum = data_15_column_sum + i
            num += 1
    Entropy_A_mean_values = data_15_column_sum / num
    data.Entropy_A[np.isnan(data.Entropy_A)] = Entropy_A_mean_values
    for i in data_numpy[:, 16]:
        if math.isnan(i):
            pass
        else:
            data_16_column_sum = data_16_column_sum + i
            num += 1
    Entropy_D_mean_values = data_16_column_sum / num
    data.Entropy_D[np.isnan(data.Entropy_D)] = Entropy_D_mean_values
    num = 0
    for i in data_numpy[:, 32]:
        if math.isinf(i):
            pass
        else:
            data_32_column_sum = data_32_column_sum + i
            num += 1
    Inverse_mean_values = data_32_column_sum / num
    data[np.isinf(data)] = Inverse_mean_values
    return data, Entropy_A_mean_values, Entropy_D_mean_values, Inverse_mean_values


def get_normalized_data_oneSignal(input_data):
    data, EA_mean_values, ED_mean_values, In_mean_values = data_deal()
...
def recongnize(test_path):
    ...
    i = 0
    pre_result = []
    model_path = r'..\python3.5\controller\Pico_controller\model_saved\model_demo.py'  # 获取模型所在目录
    unuse, Entropy_A_mean_values, Entropy_D_mean_values, Inverse_mean_values = data_deal()
    col_mean = np.zeros(38)
    col_mean[15] = Entropy_A_mean_values
    col_mean[16] = Entropy_D_mean_values
    if input.shape is ():
        print(input)
        data = data_read(np.str(input))
        # data_cut = var_adap_start(data)  # 信号截取
        texture_feature = texture_feature_extraction(data)
        general_feature = general_feature_extraction(data)

        sum_features = general_feature + texture_feature  # 特征提取，此时的两种特征提取函数和训练时不同，仅仅提取一个信号的特征
        sum_features = np.array(sum_features, np.float)
        nan_index = np.where(np.isnan(sum_features))
        sum_features[nan_index] = np.take(col_mean, nan_index)
        sum_features[np.isinf(sum_features)] = Inverse_mean_values
...
def configuration(mat_path,length,class_name = 'test_device'):
    ...
    :param: class_name: txt文件名，文件格式为test_device_1、test_device_2...
            mat_path: pico存mat文件路径（文件夹）
            txt_path: mat存txt文件路径（文件夹）,存入数据库后清空文件,也可手动到目录下删除
...
if __name__ == '__main__':
    # class_name = 'test'  # txt文件名，文件格式为test_device_1、test_device_2...
    mat_path = r'..\interference_file\matfile'  # pico存mat文件路径
    # txt_path = r'H:\ProgramData\pico_jicheng\new_test_data'  # mat存txt文件路径,存入数据库后清空
    # test_path = r'H:\ProgramData\pico_jicheng\test_path\test_path.txt'  # txt_path中选取lenth个文件，并把路径存成一个txt
    length = 10  # 读取文件夹的数据文件数量，可在界面手动更改
    # target_dir = r'H:\ProgramData\pico_jicheng\storage_test_data'  # 分类存储的文件夹的根目录
    pre_result = configuration(mat_path,length)
    print(pre_result)
