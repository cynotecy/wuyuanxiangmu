...
def compute_glcm_metrics(p, p_x,  p_y, p_xpy, p_xmy, N_g):
    ...
    if math.isinf(metrics_vect['Inverse Difference moment']):
        ...
        metrics_vect['Inverse Difference moment'] = 16388.025297739547  #16388.025297739547
    else:
        pass
    #(6)Sum average
...
def recongnize(test_path):
    ...
    #         normal_feature_oneSignal = get_normalized_data_oneSignal(
    #             sum_features)  # print("特征归一化之后：", normal_feature_oneSignal)
    #         normal_feature_oneSignal = normal_feature_oneSignal.values.reshape(1, 38)
    #         father_path = os.path.abspath('.')
    #         model_path = r'..\python3.5\controller\Pico_controller\model_saved\AdaBoost_GBDT_v1.py'  # 获取模型所在目录
    #
