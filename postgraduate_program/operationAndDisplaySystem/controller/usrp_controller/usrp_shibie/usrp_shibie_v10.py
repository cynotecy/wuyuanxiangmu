...
from controller.usrp_controller.usrp_shibie.component.freqPointList import FreqPointList
# from freqPointList import FreqPointList
...
def prior_probability(signal_freq, batch_size, P, freq_point_list):
    ...
    for key in user_dict.keys():
        ...
        if signal_freq in Interval(freql, freqr):
            if (freql in Interval(0.99*87, 108*1.01)) & (freqr in Interval(0.99*87, 108*1.01)):
                # radio_name = '调频广播'  # 暂时不管用户表单定义的名字
                u = P / 1.96e-6
                prior_p[:, 12] = 1.5
                return prior_p * u

            elif (freql in Interval(0.99*450, 470*1.01)) & (freqr in Interval(0.99*450, 470*1.01)):
                # radio_name = '农村无线接入'
                return prior_p * u

            elif (freql in Interval(0.99*470, 806*1.01)) & (freqr in Interval(0.99*470, 806*1.01)):
                # radio_name = '数字电视、微波接力'
                return prior_p * u

            elif (freql in Interval(0.99*825, 835*1.01)) & (freqr in Interval(0.99*825, 835*1.01)):
            ...
            elif (freql in Interval(0.99*870, 880*1.01)) & (freqr in Interval(0.99*870, 880*1.01)):
            ...
            elif (freql in Interval(0.99*885, 890*1.01)) & (freqr in Interval(0.99*885, 890*1.01)):
                # radio_name = 'EGSM900上行'
                u = P / 6.08e-08
                prior_p[:, 13] = 5
                return prior_p * u

            elif (freql in Interval(0.99*890, 909*1.01)) & (freqr in Interval(0.99*890, 909*1.01)):
                # radio_name = 'GSM900上行'
                u = P / 1.2141e-06
                prior_p[:, 13] = 5
                return prior_p * u

            elif (freql in Interval(0.99*915, 917*1.01)) & (freqr in Interval(0.99*915, 917*1.01)):
                # radio_name = 'ISM频段'
                return prior_p * u

            elif (freql in Interval(0.99*930, 935*1.01)) & (freqr in Interval(0.99*930, 935*1.01)):
                # radio_name = 'EGSM900下行'
                u = P / 0.113
                prior_p[:, 13] = 5
                return prior_p * u

            elif (freql in Interval(0.99*935, 960*1.01)) & (freqr in Interval(0.99*935, 960*1.01)):
                # radio_name = 'GSM900下行'
                u = P / 0.026
                prior_p[:, 13] = 5
                return prior_p * u

            elif (freql in Interval(0.99*960, 1215*1.01)) & (freqr in Interval(0.99*960, 1215*1.01)):
                # radio_name = '航空导航'
                return prior_p * u

            elif (freql in Interval(0.99*1215, 1260*1.01)) & (freqr in Interval(0.99*1215, 1260*1.01)):
                # radio_name = '科研、定位、导航'
                return prior_p * u

            elif (freql in Interval(0.99*1260, 1300*1.01)) & (freqr in Interval(0.99*1260, 1300*1.01)):
                # radio_name = '空间科学、定位、导航'
                return prior_p * u

            elif (freql in Interval(0.99*1300, 1350*1.01)) & (freqr in Interval(0.99*1300, 1350*1.01)):
                # radio_name = '航空导航、无线电定位'
                return prior_p * u

            elif (freql in Interval(0.99*1427, 1525*1.01)) & (freqr in Interval(0.99*1427, 1525*1.01)):
                # radio_name = '点对点微波通信'
                return prior_p * u

            elif (freql in Interval(0.99*1559, 1626*1.01)) & (freqr in Interval(0.99*1559, 1626*1.01)):
                # radio_name = '航空、卫星导航'
                return prior_p * u

            elif (freql in Interval(0.99*1660, 1710*1.01)) & (freqr in Interval(0.99*1660, 1710*1.01)):
                # radio_name = '气象卫星通信'
                return prior_p * u

            elif (freql in Interval(0.99*1710, 1745*1.01)) & (freqr in Interval(0.99*1710, 1745*1.01)):
                # radio_name = 'GSM1800上行'
                u = P / 8.2364e-07
                prior_p[:, 13] = 3
                return prior_p * u

            elif (freql in Interval(0.99*1755, 1780*1.01)) & (freqr in Interval(0.99*1755, 1780*1.01)):
            ...
            elif (freql in Interval(0.99*1785, 1805*1.01)) & (freqr in Interval(0.99*1785, 1805*1.01)):
                # radio_name = '民航'
                return prior_p * u

            elif (freql in Interval(0.99*1805, 1840*1.01)) & (freqr in Interval(0.99*1805, 1840*1.01)):
                # radio_name = 'GSM1800下行'
                u = 1
                prior_p[:, 13] = 15
                return prior_p * u

            elif (freql in Interval(0.99*1850, 1875*1.01)) & (freqr in Interval(0.99*1850, 1875*1.01)):
            ...
            elif (freql in Interval(0.99*1880, 1900*1.01)) & (freqr in Interval(0.99*1880, 1900*1.01)):
                # radio_name = 'TD-SCDMA'
                u = P / 0.00581
                prior_p[:, 22] = 5
                prior_p[:, 18] = 3
            ...
            elif (freql in Interval(0.99*1920, 1935*1.01)) & (freqr in Interval(0.99*1920, 1935*1.01)):
            ...
            elif (freql in Interval(0.99*1940, 1955*1.01)) & (freqr in Interval(0.99*1940, 1955*1.01)):
                # radio_name = 'WCDMA上行'
                u = P / 2.066e-06
                prior_p[:, 11] = 3
                prior_p[:, 23] = 3
            ...
            elif (freql in Interval(0.99*1980, 2010*1.01)) & (freqr in Interval(0.99*1980, 2010*1.01)):
                # radio_name = '卫星通信'
                return prior_p * u

            elif (freql in Interval(0.99*2010, 2025*1.01)) & (freqr in Interval(0.99*2010, 2025*1.01)):
                # radio_name = 'TD-SCDMA'
                u = P / 9.2889e-07
                prior_p[:, 11] = 5
                prior_p[:, 18] = 5
            ...
            elif (freql in Interval(0.99*2110, 2125*1.01)) & (freqr in Interval(0.99*2110, 2125*1.01)):
            ...
            elif (freql in Interval(0.99*2130, 2145*1.01)) & (freqr in Interval(0.99*2130, 2145*1.01)):
                # radio_name = 'WCDMA下行'
                u = P / 0.00214
                prior_p[:, 11] = 1
                prior_p[:, 23] = 3
            ...
            elif (freql in Interval(0.99*2300, 2390*1.01)) & (freqr in Interval(0.99*2300, 2390*1.01)):
                # radio_name = 'TD-LTE'
                u = P / 0.00933
                prior_p[:, 18] = 10
                prior_p[:, 23] = 10
            ...
            elif (freql in Interval(0.99*2400, 2483.5*1.01)) & (freqr in Interval(0.99*2400, 2483.5*1.01)):
                # radio_name = 'WLAN'
                u = P / 1.10046e-05
                prior_p[:, 11] = 3
                prior_p[:, 15] = 6
            ...
            elif (freql in Interval(0.99*2500, 2535*1.01)) & (freqr in Interval(0.99*2500, 2535*1.01)):
                # radio_name = '卫星广播'
                return prior_p * u

            elif (freql in Interval(0.99*2555, 2655*1.01)) & (freqr in Interval(0.99*2555, 2655*1.01)):
                # radio_name = 'TD-LTE'
                u = P / 2.90303e-05
                prior_p[:, 11] = 5
                prior_p[:, 18] = 5
            ...
            elif (freql in Interval(0.99*5725, 5850*1.01)) & (freqr in Interval(0.99*5725, 5850*1.01)):
                # radio_name = '5Ghz无线电波'
                return prior_p * u
    ...
    return prior_p * u  # 如果信号频率不存在用户表定义的频率中，返回null


def freq_point_check(signal_freq, freq_point_list):
    """
    实现频点查表功能
    :param signal_freq: 查表信号的频率
    :param freq_point_list: 用户自定义频点列表
    :return: 对应业务名称，如果查不到返回'null'字符串
    """
    user_dict = freq_point_list.getPointList()
    # 字典按频率间隔的起点排序；返回一个列表，元素为字典键值对组成的元组
    service_freq_list = sorted(user_dict.items(), key=lambda s: s[1][0])
    for key_value in service_freq_list:
        name = key_value[0]      # 业务名称
        value = key_value[1]    # 对应业务起始点频率列表
        freql = value[0]        # 起点频率
        freqr = value[1]        # 终止频率
        if signal_freq in Interval(0.99*freql, freqr*1.01):
            return name

    return 'null'


def model_test(model, inputs, freq_point_list, harmonic_num):
    """

    :param model: 传入模型
    :param inputs: 数据格式：100*4096矩阵，(一个列表，存放100个4096长度向量）
    :param freq_point_list: 用户的频点列表
    :param harmonic_num: 想要查表的谐波数量
    :return: 一个列表["中心频点(单位M)", "采样率", "调制方式:概率", dict{"频点": "业务名称", ...}
    """
    test_input, freq, bandwidth, P = inputs
    bandwidth = float(bandwidth)
    batch_size = test_input.size(0)
    signal_frq = float(freq) / 1e6
    freq_point_dict = {}  # 基波及谐波的业务字典
    ...
    with torch.no_grad():
        ...
        if harmonic_num == 0:
            # 基波识别并查表
            prior_p = prior_probability(signal_frq, batch_size, P, freq_point_list)
            freq_point_dict[str(signal_frq)] = freq_point_check(signal_frq, freq_point_list)
            out = out + prior_p
            ...
            for i in range(harmonic_num):
                name = freq_point_check(signal_frq * (i + 1), freq_point_list)
                freq_point_dict[str(signal_frq * (i + 1))] = name
        ...
        if len(top_3) == 1:
            ...
            result_list.append(freq_point_dict)
            return result_list
...
def play(freq_point_list, file_path, harmonic_num):
    """

    :param freq_point_list: 频点列表
    :param file_path: 信号文件路径
    :param harmonic_num: 想要查表的谐波数量，
    0-只识别基波并查找，1-对二倍频谐波查表，2和3以此类推，若超过3，最多查找到四倍频
    :return: 一个列表["中心频点(单位M)", "采样率", "调制方式:概率", dict{"频点": "业务名称", ...}
    """
    net = ResNet34()
    logdir = model_path
    ...
    result = model_test(model=model, inputs=input_data, freq_point_list=freq_point_list, harmonic_num=harmonic_num)
    return result
...
if __name__ == '__main__':
    ...
    result_test = play(freq_ordered_dict,
                       r"C:\Users\xmw17\OneDrive\桌面\五院项目文档\IQ数据文件\oc_collect_20200908112138.txt",
                       2)
    print(result_test)
