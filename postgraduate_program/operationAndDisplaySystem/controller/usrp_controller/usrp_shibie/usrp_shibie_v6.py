...
def extract_data_from_txt(data):
	"""
	提取3900采样数据，转换成usrp的数据格式
	3900采样数据格式: 
	...
	"""
	first_line = list(data[0, :])
	third_line = list(data[2, :])
...
def play(freq_point_list, file_path):
    ...
    if not ('.txt' in file_path):
        ...
        data = np.loadtxt(file_path, dtype=str, delimiter=' ')[0:-1] # 输入为txt文件
        data = extract_data_from_txt(data)
...
if __name__ == '__main__':
    ...
    result_test = play(freq_ordered_dict, r"D:\myPrograms\CASTProgram\postgraduate_program\data\usrp_recvfiles\single_collect_20191219124833.txt")
    print(result_test)
