...
def model_test(model, inputs, freq_point_list, harmonic_num):
    ...
    with torch.no_grad():
        ...
        if harmonic_num == 0:
            ...
            freq_point_dict[str(signal_frq)] = radio_name
            out = out + prior_p
            ...
            for i in range(harmonic_num):
                p, radio_name = prior_probability(signal_frq * (i + 2), batch_size, P, freq_point_list)
                freq_point_dict[str(signal_frq * (i + 2))] = radio_name
