...
def read_file(file_path):
    ...
    return torch.from_numpy(test_data).float(), data_freq, bandwidth

def test_model(model, logdir, inputs):
    # print('Evaluating model...')
    model.load_state_dict(torch.load(logdir))
    model.eval()
    test_input, freq, bandwidth = inputs
    ...
    with torch.no_grad():
        ...
        if signal_freq in Interval(85, 107):  # 广播
            prior_p[:, 12] = 0.5
        ...
        elif signal_freq in Interval(935, 954):  # GSM900
            prior_p[:, 13] = 5

        elif signal_freq in Interval(1785, 1805):  # 民航
            pass

        elif signal_freq in Interval(1805, 1830):  # 移动GSM1800
            prior_p[:, 13] = 5

        elif signal_freq in Interval(1830, 1840):  # 联通GSM1800
            prior_p[:, 13] = 30

        elif signal_freq in Interval(1850, 1860):  # 联通FDD下行
            prior_p[:, 22] = 30
            prior_p[:, 18] = 30
            prior_p[:, 23] = 30

        elif signal_freq in Interval(1860, 1875):  # 电信FDD下行
            prior_p[:, 22] = 30
            prior_p[:, 18] = 30
            prior_p[:, 23] = 30
        ...
        elif signal_freq in Interval(2635, 2655):  # 电信TD-LTE
            prior_p[:, 11] = 30
            prior_p[:, 18] = 30
            prior_p[:, 22] = 30
            prior_p[:, 23] = 30

        elif signal_freq in Interval(3400, 3600):  # TD BAND42
            prior_p[:, 11] = 30
            prior_p[:, 18] = 30
            prior_p[:, 22] = 30
            prior_p[:, 23] = 30
