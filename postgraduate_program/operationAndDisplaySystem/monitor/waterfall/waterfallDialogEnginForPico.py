...
class WaterfallDialog(QDialog, Ui_Dialog):
    ...
    def __init__(self, usrpNum, socket, msg, fatherFilePath, pageLimit=100, dbField = 'data_path', parent=None):
    # def __init__(self, parent = None):
        super(WaterfallDialog, self).__init__(parent)

        self.logger = logging.getLogger("Main.WaterfallDialogForPico")
        # 子窗口初始化
        self.setupUi(self)
        self.setWindowTitle("{}频谱监测视图".format(usrpNum))
        startfreq = msg.split(",")[-1].split(";")[0]
        endfreq = msg.split(",")[-1].split(";")[1]
    ...
    def run(self):
        ...
        self.state = 'run'
        # 消息发送
        communicationThread = communicationProxy.CommunicationProxy(self.msg, self.zmqLocalQ, '', self.socket)
        ...
        while self.zmqLocalQ.empty():
            ...
            if reslt == "超时":
                ...
                compressT.start()
                # 调用画布刷新函数
                self.logger.debug("画布刷新成功？"+str(self.waterfallWidget._update_canvas(dbPk, [x, y])))
                # 等待存储线程结束
                compressT.join()
